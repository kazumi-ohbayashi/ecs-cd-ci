...
# Modify application environment value
P_ES_ENDPOINT=${ES_ENDPOINT:-}
sed -i -e "s/__ES_ENDPOINT__/${P_ES_ENDPOINT}/" /etc/nginx/conf.d/nginx.conf

P_ALB_ENDPOINT=${ALB_ENDPOINT:-}
sed -i -e "s/__ALB_ENDPOINT__/${P_ALB_ENDPOINT}/" /etc/nginx/conf.d/nginx.conf
